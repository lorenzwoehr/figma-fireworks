...
const explosionQueue: Array<() => void> = [];
...
function queueExplosion(explosionFn: () => void) {
  if (deletionCount < MAX_CONCURRENT_EXPLOSIONS) {
    ...
    setTimeout(() => {
      ...
      if (explosionQueue.length > 0) {
        ...
        if (nextExplosion) queueExplosion(nextExplosion);
      }
    }, 800);
  } else {
...
function calculateScaleFactor(nodeSize: number): number {
  ...
  const dampening =
    nodeSize > midpoint ? 1 - Math.log10(nodeSize / midpoint) * 0.1 : 1;
...
function calculateExplosionParameters(bounds: {
  ...
  const particleCount = Math.max(
    5,
    Math.floor(25 * Math.sqrt(baseScaleFactor))
  );

  // Smooth curve for animation duration
  const animationDuration = Math.floor(
    400 + 400 * (1 - Math.exp(-baseScaleFactor))
  );
...
function createExplosionEffect(bounds: {
  ...
  const { particleSize, particleCount, animationDuration, maxDistance } =
    calculateExplosionParameters(bounds);
    ...
    const animationPromise = (async () => {
      ...
      for (let step = 0; step <= steps; step++) {
        ...
        await new Promise((resolve) =>
          setTimeout(resolve, animationDuration / steps)
        );
      }
